# Naming — compatibility inventory (what is load-bearing, not decorative)

**The product is Artemis.** A rename to "Evie" shipped as display-only and was
**reversed on 2026-08-13**: every surface the user reads or hears says Artemis again.
`test/identity.test.mjs` enforces that in both directions.

The reversal cost nothing because the rename never touched an identifier. That is the
lesson worth keeping, and the reason this inventory outlived the rebrand: everything
below is a **compatibility anchor**. macOS keys permissions and defaults off some of
them; the rest address on-disk user data that has no migration path. Renaming any of
them is a migration, not a find-and-replace — whatever the product is called.

Rule of thumb: **display strings are free to change. Anything the OS, the filesystem,
or an already-installed build reads is not.**

## Load-bearing identifiers — do not rename casually
...
| Built app file name `Artemis.app` | `app/build.sh` output, `app/build/` | The bundle path is part of what the user (and any launch agent / login item) points at. The display name comes from `CFBundleDisplayName`; the file on disk keeps its name so existing grants, aliases and Dock entries survive a display rename in either direction. |
...
| `ARTEMIS_SYSTEM_PROMPT` const name | `server.js` | Code identifier. Its *contents* are the persona ("You are Artemis…") and are free to change; the binding name is internal. |
...
| Custom DOM events `artemis-telemetry`, `artemis-tool`; HUD line kind `"artemis"` | `public/main.js`, `public/cockpit.js`, `public/dashboardV2.js` | Internal event/key names, decoupled from the label the HUD renders for that kind. |
...
- **No "Hey Evie" model exists.** The Evie rename was display-only and never touched the
  wake stack — which is also why reversing it needed no wake work.
...
**No data migration is required, by design — in either direction.** Both the rename to
Evie and its reversal were display-only: no bundle id, defaults domain, storage key, file
path, route, port, cookie, or env var moved. An existing install picks up the current name
on the next launch with:
...
1. **Bundle id (`com.artemis.desktop` → anything else)** — the expensive one. macOS
...
4. **`ARTEMIS_*` env vars** — a rename orphans the user's `.env` silently: nothing errors,
   features just stop being configured. Would need a dual-read (`NEW_X ?? ARTEMIS_X`)
   deprecation window.
...
8. **A new wake phrase** — not a rename at all but a new model: it requires training,
